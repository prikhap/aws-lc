...
public:
    ...
    bool process() {
        ...
        // 2. Extract module content based on platform
        ...
        // 3. Find the placeholder hash in the binary
...
int main(int argc, char* argv[]) {
    ...
    for (int i = 1; i < argc; i++) {
        ...
        else if (strcmp(argv[i], "-in-object") == 0 && i + 1 < argc) {
            input_file = argv[++i];
        }
    ...
    if (!input_file || !output_file) {
        LOG_ERROR("Usage: %s -o <output> -in-object <input> [-apple]", argv[0]);
        return 1;
    }

    InjectHash injector(input_file, output_file, apple_flag);
